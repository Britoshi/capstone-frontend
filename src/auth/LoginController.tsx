...
type RightChildProps = { onSuccess: (kind: Kind) => void };
...
const Modal: React.FC<React.PropsWithChildren<{ onClose: () => void; onOk?: () => void }>> =
    ({onClose, onOk, children}) =>
    {
        ...
        const [successKind, setSuccessKind] = useState<null | Kind>(null);
        const [successAnim, setSuccessAnim] = useState(false);
        const successRunning = successKind !== null;

        // mount → visible (next frame)
        useEffect(() =>
        {
        ...
        // start success headline animation one frame later
        useEffect(() => {
            if (successKind) {
        ...
        // success flow: expand → hold → fade out → resolve ok
        useEffect(() =>
        {
        ...
        // Split children: first = left, second = right (LoginForm)
        const nodes = React.Children.toArray(children);
        const left = nodes[0] ?? null;
        let right = nodes[1] ?? null;

        // Typed clone: inject onSuccess(kind) → no 'any'
        if (React.isValidElement<RightChildProps>(right))
        {
            right = React.cloneElement<RightChildProps>(right, {
                onSuccess: (kind: Kind) => setSuccessKind(kind)
            });
        }
                    ...
                    >
                        <h1
                            style={{
                                ...
                                transition: ["all 0.35s ease", "opacity 1.0s ease"].join(", "),
                            }}
                        >
                            {headline}
                        </h1>
