									...
									}: DesktopIconProps)
{
	...
	const openFromIcon = () =>
	{
		...
		{
			setBoxStyle((s) => ({ ...s, top: 0, left: 0, width: "100vw", height: targetHeight(), borderRadius: 0, opacity: 1 }));
			setOverlayOpacity(1);
			setContentStyle((cs) => ({ ...cs, opacity: 1, filter: "blur(0px)", clipPath: "inset(0% 0% 0% 0%)" }));
		});

		clearTimer();
		timerRef.current = window.setTimeout(() => { setPhase("open"); }, DURATION_MS);
	};
	...
	const closeToIcon = () =>
	{
		...
		if (!r) { setPhase("idle"); setOverlayOpacity(0); return; }
		...
		requestAnimationFrame(() =>
		{
			setBoxStyle((s) => ({ ...s, top: r.top, left: r.left, width: r.width, height: r.height, borderRadius: 16, opacity: 0 }));
		});

		clearTimer();
		timerRef.current = window.setTimeout(() => { setPhase("idle"); }, DURATION_MS);
	};
		...
		<>
			...
			{showWindow && (
				<div className="pointer-events-auto" style={boxStyle} onClick={(e) => e.stopPropagation()}>
					<div style={{ width: "100%", height: 44, backgroundColor: "gray", color: "black", display: "flex", alignItems: "center" }} className="px-4">
						<div className="flex justify-between items-center w-full">
							<h2 className="text-lg font-semibold">{label}</h2>
							<button onClick={closeToIcon} className="w-8 h-8 flex items-center justify-center text-zinc-500 hover:text-white hover:bg-red-600 rounded transition" title="Close">✕</button>
						</div>
					</div>
